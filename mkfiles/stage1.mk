...
$(ISO_TARGET)/.stage1-toolchain: stage1-moonbase stage1-spool
	...
	@PROMPT_DELAY=0 $(ISO_SOURCE)/scripts/chroot-build bash -c 'for mod in $(TOOLCHAIN_MODULES); do lin -rc $$mod || exit 1; done'
	@touch $@
...
$(ISO_TARGET)/.stage1: stage1-toolchain
	@echo stage1-build
	@PROMPT_DELAY=0 $(ISO_SOURCE)/scripts/chroot-build bash -c 'for mod in $(filter-out $(TOOLCHAIN_MODULES),$(STAGE1_MODULES)); do lin -rc $$mod || exit 1; done'
	@touch $@
